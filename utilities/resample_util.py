...
import LinearInterpolation as lp
...
def resample_by_resolution(image, seg, res):
    image_resize = ndimage.zoom(image, (res, res, res), order = 1)
...
def resample_labels(labels, res, interp = "nn"):
    N, height, width, depth, C = tf.shape(labels)
    x_t, y_t, z_t = grid_generator_tf(height, width, depth, res)
    
    if interp == "nn":
        labels_resize = lp.trinnsampler(labels, x_t, y_t, z_t)
    elif interp == "linear":
        labels_resize = lp.trilinear_sampler(labels, x_t, y_t, z_t)
    else:
